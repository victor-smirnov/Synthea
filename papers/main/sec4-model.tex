...
batch form of Section~\ref{sec:incremental}; in the incremental form it is re-derived in every frame
from the previous conclusion and what the frame adds, and the run of re-derivations is the continuity a
human reports (Section~\ref{sec:constructions}). Nothing in the derivation depends on which form is
used.
...
for in a text or in a substrate (Section~\ref{sec:quantity}, Appendix~\ref{app:raskolnikov}): where
...
registers as being without causes. So the regress of Section~\ref{sec:legacy} neither terminates in a
homunculus nor runs forever. A budget cuts it, and the cut is a phenomenon of the same kind as the
boundary of Section~\ref{sec:observer}: from inside, a part of the agent's own causes that it registers
as fixed by nothing it can trace.

\paragraph{Two components of the residual, and their sizes.}
\label{sec:components}
The causal analysis of Section~\ref{sec:observer} is an algorithm run on an input, and its trace is
bounded by $K(\mathrm{trace}) \le K(\mathrm{algorithm}) + K(\mathrm{input}) + O(1)$. The algorithm is
short. The input is the history of the agent and of its environment, it carries almost all of the
length, and the agent cannot compress it, which is the condition under which the conclusion is drawn at
all. What the agent cannot trace about itself has therefore two components of very different size. One
is the part of its own causal history that does not fit the budget, and it is as large as that history.
The other is the tail of the regress above, bounded by $\varepsilon/(1-r)$ and so small that it lies
below the resolution of the substrate. The Observer's conclusion registers both. Its magnitude comes
from the first; the second is what answers the regress and fixes where self-modelling stops. A trade of
time for memory, which caches part of the history inside the implementation, moves length from the
input into the algorithm and leaves the core short.

This is also the point at which the psychophysical problem of
Section~\ref{sec:psychophysical} receives its physical side. What appears to the agent, the
...
they are one residual met from two sides; here the identification of Section~\ref{sec:contribution} is
...
origination there is the residual of Sections~\ref{sec:observer} and \ref{sec:regress}, a definite
...
\emph{The Observer} is the function of Section~\ref{sec:observer}: the conclusion, drawn where the
...
\emph{The Agent} is an Observer that originates causal chains from inside its boundary and takes the
...
from its standpoint, and the account needs both (Section~\ref{sec:planes}). What makes the record
bind, and so what makes an Agent out of an Observer, is the requirement stated below under
responsibility.

\emph{The Moral Agent} is an Agent with a theory of what is good for a whole that includes others,
...
All three are stated in the batch form of Section~\ref{sec:incremental}. In a consciousness that
...
Section~\ref{sec:constructions} says what the incremental forms have to deliver, and the
transformation adds no content.

\paragraph{The stack as a composition.}
...
and why Section~\ref{sec:quantity} fixes an order in which to look for it. How the composition is realized in a Transformer, where the functions are not modules but
distributed structures that the probe of Section~\ref{sec:quantity} has to find, is taken up in
Section~\ref{sec:degree}.

\paragraph{Parallels.}
Each level has a counterpart in the literature that was reached from a different direction. The
Observer corresponds to what \citet{gallagher2000} calls the minimal self, a self without temporal
...
controlling one's own actions and through them the course of events, which \citet{haggard2017}
...
reached first and moral reasoning constructed after it \citep{haidt2001} is what the account expects
...
that say how rules are recognized, changed and adjudicated \citep{hart1961}. In the terms of this
...
\paragraph{What later versions add.}
...
formalism for the named part. Appendix~\ref{app:raskolnikov} shows the three levels defined here at
work in a text where none of them is named, and it is the model for the annotated material that the
analysis of Section~\ref{sec:quantity} needs.
...
who judges \citep{ayer1936,vanroojen2023}. The account takes a part of each. Good and evil have a
...
is the Observer. To \emph{see} red, a system must first \emph{be}, in the sense of existing for itself; the order of
...
substitution went unnoticed \citep{perky1910}; the effect, the assimilation of a real signal into
imagery, has since been reproduced in other modalities \citep{okada1992}. The content is stable from frame to frame in a way the
...
\paragraph{The lamp as the two faces in one frame.}
\label{sec:lamp-balance}
The source component is the inference of Section~\ref{sec:observer} pointed outward, and
Section~\ref{sec:twofaces} says the two directions meet one residual. That suggests what the lit thing
of Section~\ref{sec:lamp} is in the framework's terms: neither the break alone, which is present in the
...
criterion of Section~\ref{sec:criterion} as the Observer does: refining the self-model moves it, as
...
classified together as one disorder \citep{apa2013,spiegel2011,sierra1998,hunter2004}. Dreamless sleep is the boundary with nothing on the world side; a dream
...
given'', is measurable by the probe of Section~\ref{sec:quantity}. We record the construction as a
proposal: it adds no new primitive, and it stands or falls with the identity of
Section~\ref{sec:twofaces}. Locke's inverted spectrum
...
machine. The third column of the table is the list of labels the probes of Section~\ref{sec:quantity}
...
70~Hz and at higher rates gives way to pitch \citep{zwicker2007,moore1995,plomp1965}; the neural
processing of amplitude modulation across these time scales is reviewed by \citet{joris2004}. The
...
Section~\ref{sec:quantity} asks, present an input varying at increasing rate and find the scale at
...
at which it generalizes, which need not coincide (Section~\ref{sec:frame}).
...
computationally that is the incremental form of Section~\ref{sec:incremental}, the conclusion
...
than the token unless the architecture feeds new input between layers (Section~\ref{sec:granularity}).
