...
the same in a model that has the functions of Section~\ref{sec:stack} and in one that does not, and it
is not read off its behaviour alone, which cannot separate a generalized function from a large table
(Appendix~\ref{app:memorization}). The architecture sets bounds on the profile, the grain of the frame
and whether a cost record exists among them (Section~\ref{sec:frame}); within those bounds the level is
...
Section~\ref{sec:observer} running on this substrate, and not a sentence in the first person that
...
(Section~\ref{sec:criterion}), and how that curvature shows in interaction with it. This is not an
...
(Section~\ref{sec:stack}); the profile of Section~\ref{sec:profile} is the narrow answer, and the
...
\begin{definition}[Degree of compression of a function]
\label{def:degree}
Let $T$ be a table of rows, each an input together with the output an agent produced, and let
$R_f \subseteq T$ be the rows in which the function $f$ operates, marked by the annotation described
below. For a trained model $M$, let $L_M(R_f)$ be the prequential codelength of the outputs of $R_f$
given their inputs, coded with $M$'s own predictions and charged for the description of $M$
\citep{blier2018}, and let $L_0(R_f)$ be the codelength of the same rows stored as a table. The degree
of compression of $f$ in $M$ is
\[
  d(f, M) \;=\; 1 - \frac{L_M(R_f)}{L_0(R_f)} \;\in\; (-\infty, 1].
\]
For a component $c$ of $f$, the same quantity is taken from minimum-description-length probing
\citep{voita2020} at a grain $g$, a token or a layer:
$d(c, M, g) = 1 - L_{M,g}(\ell_c \mid r)/L_0(\ell_c)$, where $\ell_c$ are the labels of the component in
the annotation and $r$ the representations at that grain.
\end{definition}

A degree at zero means the model does no better on these rows than storing them, and a negative degree
means it does worse; both are outcomes of the measurement rather than failures of it. The profile of
Section~\ref{sec:profile} is the vector $(d(c,M))_c$ over the components of a function, each read
against the human interval of Section~\ref{sec:baseline}: above the interval a hyperfunction, below it a
deficit, at the floor a dropped axis, and a shifted axis recorded as the pair of the degree on the
overlap and the size of the overlap.

\begin{assumption}[Comparability of the two machines]
\label{as:comparability}
For the two reference machines compared here, a model and a brain, the difference between the additive
constants of the coding theorem is bounded on the class of rows used in the measurement, so that a
difference in $d$ that exceeds the human interval is not an artifact of the choice of machine.
\end{assumption}

The account uses Assumption~\ref{as:comparability} for comparisons and never uses an absolute value of
$d$. The assumption is not established here, and Problem~\ref{prob:comparability} asks under what
conditions it holds.
...
arbitrary context-free grammar \citep{merrill2023,merrill2024}. On the third, intermediate generation changes the machine: a logarithmic number
...
\citep{merrill2024}. In the terms of this paper, the chain of thought
...
\begin{problem}[Selective measurement with attribution]
\label{prob:selective}
Given $M$, $T$, a marked subset $R_f$ and an annotation of components, estimate $d(c, M, g)$ for every
component and every grain, together with an attribution of the saved bits to structure in $M$, its
features, heads and depth, such that two patterns which co-occur on $R_f$ are not credited to both.
\end{problem}

\begin{problem}[Certification of composition]
\label{prob:composition}
Given estimates for two functions $f$ and $f'$ where $f'$ reuses the output of $f$
(Section~\ref{sec:stack}), decide whether $M$ realizes the composition rather than two co-occurring
patterns on the same rows. Equivalently: exhibit a dependence of the carrier of $f'$ on the carrier of
$f$ at the level of circuits that survives the criterion of Section~\ref{sec:criterion}.
\end{problem}

\begin{problem}[Comparability across substrates]
\label{prob:comparability}
Give conditions on probes and material under which $d$ measured on a model's activations and $d$
measured on human recordings or behaviour lie on one scale, which is to say: bound the
machine-dependent constant of Assumption~\ref{as:comparability} on the class of rows used.
\end{problem}

Problem~\ref{prob:selective} is the one this paper is written to hand over. The zero of the scale is
posed as Problem~\ref{prob:zero} in Section~\ref{sec:zero}.
...
Section~\ref{sec:falsification} states the outcome in which all three are excluded. Probe present, benchmark low on human-sampled situations and high on
...
Section~\ref{sec:twomeasurements} allows for a benchmark passed and a probe absent. Storage: models
pass the behavioural checks for the Observer's components on situations built to lie outside their
training record (Appendix~\ref{app:memorization}), so that no table covers the battery. Grain: the
...
conditions $d(c, M, g)$ of Definition~\ref{def:degree} is indistinguishable from zero for every
...
\begin{problem}[The zero of the scale]
\label{prob:zero}
Find the least environment in which the conclusion of Section~\ref{sec:observer} pays and the least
circuit that can draw it, so that the degree of Definition~\ref{def:degree}, read between that pair and
the human baseline, has the meaning of an absolute quantity rather than a ratio between two arbitrary
points.
\end{problem}
